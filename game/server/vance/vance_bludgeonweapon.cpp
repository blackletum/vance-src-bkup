	...
	if (m_bWaitingForDeathSetup && GetActivity() != ACT_VM_THROW) {
		...
		if (!pOwner->GetLastWeapon()) {
			if (!pOwner->SwitchToNextBestWeapon(this)) {
				pOwner->Weapon_Switch(static_cast<CVancePlayer *>(pOwner)->m_UnarmedWeapon); //default to unarmed weapon
			}
		} else {
...
void CThrownBludgeonProp::VPhysicsCollision(int index, gamevcollisionevent_t *pEvent) {
	...
	if (!ToBasePlayer(pHitEntity)) { //damaging the player like this crashes the game, so dont do that
